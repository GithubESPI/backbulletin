...
def fetch_with_retry(url, headers, retries=3, delay=5):
    for attempt in range(retries):
        try:
            ...
            time.sleep(delay)
    raise HTTPException(status_code=500, detail=f"Failed to fetch data from {url} after {retries} attempts")
...
async def fetch_api_data_for_template(headers):
    ...
    for url in api_urls:
        try:
            ...
            raise e  # Stop the process if any API call fails
            ...
            for page_num in range(pdf_document.page_count):
                ...
                for line in lines:
                    if "Identifiant :" in line:
                        ...
                        logger.error(f"Invalid Identifiant format in line: {line}")
                        return None
        return None
    except Exception:
...
async def upload_and_integrate(doc_urls: DocumentUrls):
    ...
    try:
        ...
        progress_data[session_id] = 15
        await update_progress(session_id, 15)

        # Télécharger le fichier Word depuis l'URL
        ...
        progress_data[session_id] = 20
        await update_progress(session_id, 20)

        # Détecter le template approprié en fonction des valeurs du fichier Excel téléchargé
        ...
        # Progression à 30%
        progress_data[session_id] = 30
        await update_progress(session_id, 30)

        # Télécharger le template correspondant depuis Azure Blob Storage
        blob_name = settings.M1_S2_MAGI_TEMPLATE  # Utiliser le nom du blob
        temp_template_path = os.path.join(settings.DOWNLOAD_DIR, "M1-S2-MAGI.xlsx")  # Spécifier le chemin de téléchargement
        azure_blob_service.download_template(blob_name, temp_template_path)

        # Utiliser le fichier téléchargé depuis Azure pour générer le bulletin
        template_wb = await process_file(uploaded_wb, temp_template_path, columns_config)
...
async def import_bulletins_from_directory():
    bulletin_dir = r"C:\Users\AndyVESPUCE\backendBulletin-main\backendBulletin-main\outputs\bulletins"
