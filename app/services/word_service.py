...
def generate_word_document(student_data, case_config, template_path, output_dir):
    ...
    # Ensure the group name is relevant
    if group_name not in settings.RELEVANT_GROUPS:
        logger.warning(f"Group {group_name} is not relevant for processing.")
        return None

    # Correct the case key if necessary
    corrected_key = case_config["key"].replace("_", "-")

    ects_data_key = corrected_key
    # Handle special cases for ECTS data
